...
import { StyleSheet, Text, View, Image, Alert, ScrollView, TouchableOpacity, Dimensions } from 'react-native';
...
export default function MentalHealthHome() {
  ...
  const fetchWeeklySummary = async (id) => {
    try {
      const response = await fetch(`http://localhost:5000/weekly-summary/${id}`);
      if (!response.ok) {
