...
import { updateMealsStatus } from '../../../server/api'; // Ensure this is your correct API function import
...
const WorkoutItem = ({
  ...
  const handlePress = () => {
    if (workoutDate === selectedDate && workout.completed === 'pending') {
      navigation.navigate('WorkoutTimerScreen', { workout });
    }
  };
  ...
  const handleCompleteToggle = async (workoutId) => {
    ...
    try {
      const response = await updateMealsStatus(
        planId,
  ...
  return (
    <TouchableOpacity style={styles.workoutCard} onPress={handlePress}>
      ...
      <View style={styles.textContainer}>
        ...
        <Text style={styles.workoutDuration}>{workout.description}</Text>
